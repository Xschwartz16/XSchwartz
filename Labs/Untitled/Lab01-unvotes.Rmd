...
```{r}
un_votes %>%
  filter(country %in% c("United States of America", "Canada","Brazil")) %>%
  inner_join(un_roll_calls, by = "rcid") %>%
  inner_join(un_roll_call_issues, by = "rcid") %>%
  group_by(country, year = year(date), issue) %>%
  summarize(
  ...
  filter(votes > 5) %>%  # only use records where there are more than 5 votes
ggplot(un_votes,mapping=aes(x=year,y=percent_yes,col=country,shape=country,linetype=country))+geom_point()+geom_smooth(se=F)+scale_color_manual(values = c("#4A9B7A","#CA6727","#7470AE"))+ 
    labs(
      title = "Percentage of 'Yes' votes in the UN General Assembly",
      subtitle = "1946 to 2015",
      y = "% Yes",
