---
title: "Lab 04 - Health care spending and coverage in the US"
...
If we're interested in the relationship between spending and coverage, we'll only be able to use observations that have information on both.  That is, we won't be using data from years for which we only have spending information or only have coverage information.  Remove any variables we won't be using from `spending` and `coverage`.

> RESPONSE: 

```{r}
spending <- spending%>%select(1,24,25)
coverage <- coverage%>%select(1,2:15)
...
```{r}
spending <- spending %>% filter(Location!="United States",Location!="District of Columbia")
coverage <- coverage %>% filter(Location!="United States",Location!="District of Columbia")
...
> RESPONSE: Joining, filtering out NAs. 
...
covSpen <- inner_join(coverage, spending, by = "Location")
covSpen <- clean_names(covSpen,"snake")
# ggplot(data = covSpen, aes) +
...
See the figure on the slide 66.  Create that plot for 2013.  (Hint: use `nudge_x = 150` in the `geom_text` layer.)
...
Done early?  Try to figure out how to make these additional updates to the first figure from the last exercise (same figure as shown on slide 66) to hone your plotting skills:
