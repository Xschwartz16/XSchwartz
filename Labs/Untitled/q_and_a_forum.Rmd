...
### QX. [#5. How does geom_curve work in Figure 3.22?  How was the shape and position of the arrow to median year of birth set?  What does “curvature = 0.5” do? ] 
...
- delete the text and section header for the # Original Code
- knit the document to pdf or html
- go to the "R Questions and Answers" forum on Moodle, find your question, click "Discuss this topic", then click "Reply"
- You can either type your response into the message box or just attach your knit pdf or html document as the solution (for most questions, the attachment will be the easiest way to go; it can be cumbersome to intersperse text and images directly into the message box)

> ANSWER: The position is set using x and y coordinates, but with an end point  ("xend" and "yend") for each point. The arrow function is used to create an arrowhead at the end of the line (with a length and unit for the size of the arrowhead). Curvature represents the bend of line, with 1 = semicircle, .5 = quarter circle, and 0 = straight line. 


```{r}
ggplot(data = MLB_teams, aes(x = W, y = WPct))+geom_point()+
geom_curve(x = 80, xend = 90, y = .4, yend = .5, arrow = arrow(length = unit(1.5,"cm")), curvature = .5)+
  geom_curve(x = 90, xend = 80, y = .5, yend = .4, arrow = arrow(length = unit(.1,"in")), curvature = .5)+
geom_curve(x = 50, xend = 75, y = .55, yend = .6, arrow = arrow(length = unit(.5,"cm")), curvature = 1)
