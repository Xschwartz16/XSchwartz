...
```{r}
#MigrationFlows2 <- summarise_at(MigrationFlows,vars( destcode:origincode))

MigrationFlowsF <- MigrationFlows %>% filter(Y2000 > 0 & sex == "Female")
MigrationFlowsM <- MigrationFlows %>% filter(Y2000 > 0 & sex == "Male")


MigrationFlows2 <- merge(MigrationFlowsF[2:4], MigrationFlowsM[2:4], all = TRUE)
library(plyr)


MigrationFlows2 <- ddply(MigrationFlows2,~destcode + origincode, summarise,Y2000=sum(Y2000))
...
top5in <- strength(mig, weights = E(mig)$Y2000, mode = "in")
top5out <- strength(mig, weights = E(mig)$Y2000, mode = "out")
...
> ANSWER: The average edges per node is 3.97. 
...
> ANSWER: The graph shows the various connections between the five countries. Thicker lines represent more connections between the country. 
...
> ANSWER: It appears to be undirected, which implies goods (money perhaps) are going in both directions. 
